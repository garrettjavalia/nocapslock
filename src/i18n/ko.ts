...
export const koMessages: Copy = {
  ...
  demoSection: {
    ...
    bodyTemplate:
      '아래 텍스트 영역에서는 [[Caps Lock]]이 [[A]], [[C]], [[V]], [[X]]에 대해 가상 [[{modifier}]] 보조키처럼 동작합니다. 시스템 전체를 바꾸기 전에 느낌부터 먼저 확인해 볼 수 있습니다. 클립보드 관련 동작은 처음 한 번 브라우저 권한을 물어볼 수 있습니다.',
    statusIdle: '대기 중',
    statusArmed: '가상 보조키 활성',
    statusPrefix: '상태',
      instructions:
      '[[Caps Lock]]을 한 번 누른 뒤 1초 안에 [[A]], [[C]], [[V]], [[X]] 중 하나를 누르면 이 데모 안에서는 [[{modifier}]] 단축키처럼 동작합니다. 단축키를 한 번 쓸 때마다 [[Caps Lock]]을 다시 눌러야 합니다.',
    restoreNote:
      '체험이 끝나면 다음 작업에 지장이 없도록 [[Caps Lock]]을 다시 원래 상태로 돌려두세요. 브라우저와 운영체제에 따라 버그가 있을 수 있습니다.',
        ...
        steps: [
          {
            ...
            body: 'PowerToys를 설치한 뒤 Keyboard Manager에서 Remap a key를 열고 [[Caps Lock]]을 원하는 키로 바꿉니다. Microsoft 문서에 따르면 PowerToys 방식은 즉시 적용되지만, PowerToys가 백그라운드에서 실행 중이어야 하고 로그인 화면에서는 동작하지 않습니다.',
          },
          {
      ...
      },
      {
        ...
        steps: [
          ...
          },
          {
            ...
            body: 'Karabiner-Elements를 열고 Simple Modifications로 들어간 뒤, 수정할 키보드를 고르고 [[Caps Lock]]을 [[Left Control]]로 바꿉니다. [[Escape]] 같은 다른 목표 키가 더 편하다면 그 키를 선택해도 됩니다.',
          },
          {
            title: '3. 선택 사항: [[Caps Lock]]이 이미 언어 전환 키일 때만 이어서 보기',
            body: '다음 단계들은 [[Caps Lock]]이 이미 입력 소스 전환 용도로 쓰이는 다국어 macOS 환경을 위한 안내입니다. 현재 [[Caps Lock]]이 그런 역할을 하고 있지 않다면, 기본 재매핑만으로 충분하므로 아래 선택 사항 단계들은 건너뛰어도 됩니다.',
          },
          {
            title: '4. 선택 사항: 오른쪽 보조키 하나 고르기',
            body: '먼저 오른쪽 보조키 중 하나를 입력 언어 전환용으로 정합니다. 보통은 [[Right Command]] -> [[F18]]이 무난하고, 필요하면 [[Right Option]] -> [[F18]]을 써도 됩니다. 다만 그 키가 현재 레이아웃에서 중요한 문자를 만드는 데 쓰이지 않는지 먼저 확인하는 편이 좋습니다.',
          },
          {
            title: '5. 선택 사항: Karabiner에서 그 오른쪽 키를 [[F18]]로 바꾸기',
            body: 'Simple Modifications에서 앞 단계에서 고른 키를 [[Right Command]] -> [[F18]] 또는 [[Right Option]] -> [[F18]]으로 추가합니다. 이렇게 하면 [[Caps Lock]]을 재매핑용으로 비우면서도, 키보드 오른쪽에 입력 언어 전환 전용 키를 따로 둘 수 있습니다.',
          },
          {
            title: '6. 선택 사항: macOS에서 [[F18]]을 입력 소스 전환 키로 배정',
            body: 'Apple 메뉴 > 시스템 설정 > 키보드 > 키보드 단축키 > 입력 소스로 들어간 뒤, “입력 메뉴에서 다음 소스 선택” 항목에 [[F18]]을 지정합니다. 그러면 [[Caps Lock]]을 다른 역할로 바꾼 뒤에도 입력 언어 전환 키를 유지할 수 있습니다. 여기서 [[F18]]을 권하는 이유는 Apple이 상단 [[F1]]부터 [[F12]]까지의 키를 기본 시스템 기능과 연결하고 있고, Karabiner도 이 범위의 기능키에는 별도 처리가 필요한 경우가 있다고 안내하기 때문입니다.',
          },
        ],
        notes: [
          '저장 전에 실제 키 이름을 확인하고 싶다면 Karabiner 공식 도구인 EventViewer를 쓰는 것이 가장 정확합니다.',
          'Apple은 [[Caps Lock]]으로 마지막으로 사용한 라틴 입력 소스와 전환하는 기본 기능도 문서화하고 있습니다. 하지만 [[Caps Lock]] 자체를 재배치하고 싶다면, [[Right Command]] -> [[F18]] 경로가 더 유연합니다.',
          sharedGuideCopy.ko.keepCapsLockNote,
        ],
        placeholder: '',
      },
...
esc = capslock`,
        ...
        steps: [
          ...
          },
          {
            ...
            body: '이어서 /etc/keyd/default.conf를 작성합니다. 아래 예시처럼 가장 단순한 overload 설정부터 시작하면 [[Caps Lock]]을 탭했을 때는 [[Escape]], 누르고 있을 때는 [[Control]]로 동작하게 만들 수 있습니다.',
          },
          {
            title: '3. 데몬 다시 불러오기',
          ...
          {
            title: '4. 키 이름 확인',
            body: '키보드가 특이하거나 키 이름이 헷갈릴 때는 keyd monitor로 입력을 살펴보세요. 원래 입력 이벤트를 확인하고 싶다면 keyd를 잠시 멈춘 뒤 다시 살펴보면 됩니다.',
          },
          {
